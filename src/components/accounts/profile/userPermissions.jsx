...
import { CheckSquare, ChevronUp, LoaderCircle, Square, Trash, Trash2, X } from 'lucide-react'
...
const UserPermissions = ({ togglePermissions, userId }) => {
    ...
    const handleRemoveUserFromGroups = async () => {
        ...
        try {
            ...
            setPermissions(permissions.filter((perm) => !selectedIds.includes(perm.id)));
            setSelectedGroups([]);
        } catch (error) {
    ...
    return (
        <div className='popup'>
            <div className="popup-content" style={{
                ...
                <p>These are the permission groups and permissions that user have on certain features</p>
                {
...

                {
                    ...
                    >
                        {
                            isRemoving ? <LoaderCircle className='loading-icon' size={18} />
                                : <Trash2 size={18} />
                        }
                        Remove permissions
                    </button>
                                ...
                                ? <div className="user-permissions-list form">
                                    {
                                        permissions.map((permission, index) => (
                                            <div onClick={() => handleToggleFeature(permission.name)} className={`dropdown ${featureToShow === permission.name && 'show'}`}>
                                                <div className="header">
                                                    <div className="action" onClick={(e) => {
                                                        e.stopPropagation(); handleSelectGroup({
                                                            id: permission.id,
                                                ...
                                                <div className="content perms" style={{ flexDirection: 'unset' }}>
                                                    {
