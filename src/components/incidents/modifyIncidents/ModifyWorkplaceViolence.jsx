...
const ModifyWorkplaceIncident = ({ data }) => {
    ...
    // forms
    const [severityRating, setSeverityRating] = useState(incident?.severity_rating);
    ...
    const [suggestions, setSuggestions] = useState(incident?.prevention_suggestion);
    const [explainselfinjury, setExplainSelfInjury] = useState(
        incident?.type_of_incident && JSON.parse(incident?.type_of_incident).explanation
    );
    ...
    const [selectedBackground, setSelectedBackground] = useState(
        (incident?.type_of_incident && JSON.parse(incident?.type_of_incident).incidents) || []
    );
    ...
    const [reportedTitle, setreportedTitle] = useState(incident?.reported_by_title);
    ...
    const [terminationIncidents, setTerminationIncidents] = useState(initialTerminationIncidents);
    ...
    const handleModify = async (incidentStatus) => {
        ...
        console.log("Person Injured  ", injuries.map((injury) => ({
            first_name: injury.first_name,
            last_name: injury.last_name,
            injury_description: injury.injury_description,
            profile_type: "Victim",
