...
const ModifyWorkplaceIncident = ({ data }) => {
  ...
  const [selfInjury, setSelfInjury] = useState(() => {
    try {
      if (incident?.type_of_incident) {
        const parsedData = JSON.parse(incident.type_of_incident);
        return (
          Array.isArray(parsedData.incidents) &&
          parsedData.incidents.includes("Other Types")
        );
      }
      return false;
    } catch (error) {
      console.error("Error parsing type_of_incident:", error);
      return false;
    }
  });
  ...
  const [explainselfinjury, setExplainSelfInjury] = useState(() => {
    try {
      ...
      console.error("Error parsing type_of_incident for explanation:", error);
      return "";
    }
  });
  ...
  const [selectedBackground, setSelectedBackground] = useState(() => {
    try {
      if (incident?.type_of_incident) {
        const parsedData = JSON.parse(incident.type_of_incident);
        return Array.isArray(parsedData.incidents) ? parsedData.incidents : [];
      }
      return [];
    } catch (error) {
      console.error("Error parsing type_of_incident for incidents:", error);
      return [];
    }
  });
