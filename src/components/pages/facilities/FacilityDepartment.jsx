...
const FacilityDepartmentContent = () => {
    ...
    const getDepartment = async () => {
      try {
        ...
        const res = await api.get(
          `/departments/${department_id}/?expand=members,facility`
        );
        if (res.status === 200) {
    ...
    const fetchFacility = async () => {
      try {
        ...
        const response = await api.get(
          `/facilities/${facility_id}/?expand=departments`
        );
  ...
  useEffect(() => {
    const fetchStaffMembers = async () => {
      try {
        const response = await api.get(
          `/facilities/departments/${department_id}/members/`
        );
        if (response.status === 200) {
  ...
  useEffect(() => {
    const getIncidents = async () => {
      try {
        const response = await api.get(
          `/facilities/departments/${department_id}/incidents`
        );
        if (response.status === 200) {
        ...
        <div className="facility-department-details-row">
              ...
              {department ? (
                <>
                  ...
                  <p className="date-created">
                    Created on{" "}
                    <DateFormatter dateString={department.created_at} />
                  </p>
                </>
              ) : (
          ...
          <div className="col-3">
            ...
            >
              ...
              {permissions &&
              permissions.base?.includes("change_department") &&
              permissions.base?.includes("delete_department") ? (
