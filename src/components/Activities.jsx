...
const IncidentActivitiesTab = ({ incidentId, incident_type, setCount }) => {
    ...
    const handleFetchActivities = async () => {
        setIsLoading(true);
        const response = await fetchActivities(incidentId, incident_type);
        if (!response.success) {
    ...
    const handleActivityLink = (activity) => {
        ...
        if (activity.activity_type === 'incident_sent' && activity.activity_highlight?.id) {
            return `/departments/${activity.activity_highlight.id}`;
        } else if (activity.activity_type === 'incident_closed' || activity.activity_type === 'document_uploaded') {
                                    ...
                                    <div className="user-destination">
                                        ...
                                        }
                                        {
                                            activity.created_by && (
                                                <div className="user">
                                                    <small>By:</small>
                                                    <span>{activity.created_by.first_name} {activity.created_by.last_name}</span>
                                                </div>
                                            )
                                        }
                                        {
                                            ...
                                            <div className="files-list">
                                                {activity.files.map((file, index) => (
                                                    <div key={index} className='document'>
                                                        <a className='file' href={file.url} target="_blank" rel="noopener noreferrer">
                                                            <img src={`/images/files/types/${file.file_type}.svg`} alt="" />
                                                            <span>{file.name}</span>
